...
import WalletPopover from 'components/common/WalletPopover/WalletPopoverV2';
import { Context as FeedbackContext } from 'context/FeedbackContext';
import { Context as AuthContext } from 'context/AuthContext';
import { isMobile } from 'react-device-detect';
import { Box } from '@mui/system';
import ICWallet from 'assets/wallet.svg';
import { getWalletMe, registerWallet } from 'apiProvider';
import styles from './WalletButtonV2.module.scss';
...
const WalletButtonV2 = () => {
  ...
  return (
    <>
      <WalletButtonBase
        ref={anchorEl}
        sx={{ height: 44, color: 'var(--whiteColor)' }}
      ...
      >
        {walletInfo.loading ? (
          ...
          <>
            <span className={styles.walletIconMobile}>
              {walletInfo.name ? getIcon(walletInfo.name) : <ICWallet />}
            </span>
            <span className={styles.walletLabel}>{buttonText}</span>
          </>
        )}
      </WalletButtonBase>
      <WalletPopover
