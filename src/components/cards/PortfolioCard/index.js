...
const Card = ({
  ...
  const {
    state: { walletInfo, localFilters, isMobile },
  } = useAppContext();
      ...
      <CardActionArea>
        <CardContent style={{ padding: '10px' }}>
          ...
          <Box className={styles.description}>
            ...
            <CustomTooltip
              title={
                deal_coefficient && showValuation && price
                  ? `${valueCoefficientText} with a Jungle Value Index(JVI) of ${trueCoefficient?.toFixed(
                      2
                    )}`
                  : ''
              }
                  ...
                  {loading || sortOrFilerLoading ? (
                    ...
                    <>
                      <Box className={styles.title}>
                        {marketplace || sale_marketplace
                          ? (marketplace || sale_marketplace).toUpperCase()
                          : 'Price'}
                      </Box>
                      {!price && !sale_price ? (
                        <Box
