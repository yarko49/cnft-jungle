...
const CollectionTraits = ({
  ...
  const fetchTraits = async () => {
    try {
      ...
      if (!response.success || !response.traits.length) {
        return [];
      }
  ...
  const handleSelectedTraits = async (newTraits) => {
    if (newTraits.length > 5 && user.snipeTier !== 'orca') {
      return showFeedback({
        kind: 'error',
        message: 'Your permissions can only select five traits!',
        duration: 2000,
      });
    }
  ...
  return (
    <Autocomplete
      ...
      groupBy={(option) => capitalize(option.split('/')[0])}
      ...
      renderOption={(props, option, { selected }) => (
        <li {...props}>
          ...
          />
          {capitalize(option)} (
          {percentages[option] ? `${percentages[option]}%` : ''})
        </li>
      )}
      renderInput={(params) => (
        <TextField
